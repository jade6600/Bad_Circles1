...
def mover_villano():
    if not jugador1_vivo and not jugador2_vivo:
        return
    # Elegir el jugador más cercano para matarloooooooo
...
def chequear_colisiones():
    ...
    # si ambos muertos,la mala
    
    if not jugador1_vivo and not jugador2_vivo:
      guardar_puntaje(puntaje_total)
      estado = GAME_OVER
    if jugador1_vivo or jugador2_vivo:
       if len(frutas) < 5:  
        for _ in range(5 - len(frutas)):
            x = random.randint(0, ANCHO - 40)
            y = random.randint(0, ALTO - 40)
            frutas.append(pygame.Rect(x, y, 40, 40))
