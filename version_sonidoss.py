    ...
    if jugador2_vivo:
        for f in frutas[:]:
            if jugador2.colliderect(f):
                frutas.remove(f)
                puntaje_total += 1
    # Villano colisiona con los jugadores 
...
def dibujar_instrucciones():
    ...
    lineas = [
        ...
        "Recolecta todas frutas para ganar puntos.",
...
def dibujar_puntajes():
    ...
    y = 150
    if not puntajes:
        txt = fuente_info.render("No hay puntajes todaviaaaa.", True, BLANCO)
...
pygame.quit()
# Se acabo este programita pero esta en proceso creativooo.